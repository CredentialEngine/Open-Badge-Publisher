<script lang="ts">
	...
	const handleSaveRow = () => {
		...
		// validate value
        try {
            value.map(v => {validator.validateSync(v);})
        } catch (err) {
...
{#if !isEditing} <!-- Display the Value -->
	<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
		<th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
			{fieldName || fieldId}
		</th>
		<td class="py-4 px-6">
			<slot>{#each value as valueEntry (valueEntry)}<Tag>{abbreviate(valueEntry)}</Tag>{/each}</slot>
		</td>
		<td class="py-4 px-6">
			{#if editable}
				<button
		...
		<td class="py-4 px-6">
                <div class="mb-3">
                    {#each value as valueEntry (valueEntry)}
                        <Tag>
                            <span class="text-sm">
                                {abbreviate(valueEntry)}
                                <button on:click={ () => {handleRemoveValue(valueEntry);} } type="button" class="inline-flex items-center p-0.5 ml-2 text-xs text-gray-400 bg-transparent rounded-sm hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-300 dark:hover:text-gray-900" data-dismiss-target="#badge-dismiss-dark" aria-label="Remove">
                ...
                <div class="flex items-end flex-col md:flex-row space-x-3">
                    <input
                        id={inputId}
                    ...
                    <button
                        ...
                        on:click={ () => { handleAddValue(inputValue);} }
                    >
				        Add
			        </button>
