	...
	{#if !ac.skip}
		<div transition:slide class="mb-4">
			<p class="text-xs text-midgray">
				...
				{#if ac.sourceData.targetFramework}<br />Framework name: {ac.sourceData
						.targetFramework}{/if}
			</p>
			...
			<p class="text-xs text-midgray my-2">
				{#if propertyType == 'DEFAULT'}
					{getPropertyName(nodeType, $publisherOptions.alignmentSettings.defaultPropertyType)} (default):
				{/if}
