	...
	<div class="relative w-full h-full max-w-2xl mx-auto h-auto">
		<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
			...
			<div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
				<slot name="heading"><Heading><h4 class="!mt-0">{title}</h4></Heading></slot>
				<button
			...
			<!-- Modal footer -->
			<div
				class="flex flex-row justify-end p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600"
			>
				{#each actions as action}
					<Button buttonType={action.buttonType} on:click={action.onClick}>{action.label}</Button>
				{/each}
			</div>
		</div>
	</div>
