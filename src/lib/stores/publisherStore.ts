...
export const publisherCredentials = writable<PublisherCredentialsDataStore>({credentials: [], totalResults: 0})
...
export const publisherOptions = writable<PublisherOptions>({cookieBasedAccess: false, apiKey: '', accessToken: undefined})

// Governs which step is displayed
export const publisherSetupStep = writable<Number>(0)

export const getUser = async () => {
    if ( !browser || get(publisherUser).user )
        return null

    const url = `${PUBLIC_UI_API_BASEURL || '/publisher'}/StagingApi/Load/User`
    if ( !haveSameDomain(url, PUBLIC_BASEURL) )
        return null
