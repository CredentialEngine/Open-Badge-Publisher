<script lang="ts">
	...
	import { parchmentRegions } from '$lib/utils/parchment.js';

	let saveAllTriggered = false;

	const updateParchmentCredential = async (c: CtdlApiCredential): Promise<boolean> => {
		const parchmentBadge = $parchmentSelectedIssuerBadges.find(
			(pb) => pb.openBadgeId == c.Credential.CredentialId
		);
		const ctid = $ctdlPublicationResultStore[c.Credential.CredentialId]?.CTID;

		if (!parchmentBadge) throw new Error('Could not identify source badge in Parchment.');
		...
		const requestBody = {
			name: parchmentBadge.name,
			description: parchmentBadge.description,
			alignments: [
				...parchmentBadge.alignments.filter((a) => a.targetUrl !== newAlignment.targetUrl),
				newAlignment
			]
		};

		const requestData = {
			URL: `${parchmentRegions.get($parchmentSelectedRegion || 'test')?.apiDomain}/v2/badgeclasses/${
				parchmentBadge.entityId
			}`,
			Method: 'PUT',
	...
	const processNextCredential = () => {
		...
		if (!!nextCredential) {
			updatePromises[nextCredential.Credential.CredentialId] =
				updateParchmentCredential(nextCredential);
...
<BodyText>
	Update your badges in Parchment to add alignments to Credential Finder pages. This helps
...
<div class="py-4">
	<Button
		...
		disabled={saveAllTriggered}>Save all alignments to Parchment</Button
	>
</div>
	...
	<table class="w-full text-sm text-left text-gray-500">
		...
		<tbody>
			{#each credentials as credential}
				{@const parchmentBadge = $parchmentSelectedIssuerBadges.find(
					(pb) => pb.openBadgeId == credential.Credential.CredentialId
				)}
				<tr class="bg-white border-b">
					...
					<td class="py-4 px-6">
						{#if parchmentBadge?.alignments?.find((a) => a.targetUrl == alignmentUrlForCredential(credential.Credential.CTID))}
									...
									<Button
										on:click={() => {
											updatePromises[credential.Credential.CredentialId] =
												updateParchmentCredential(credential);
										}}>Save</Button
									>
								{/if}
							{:catch error}
