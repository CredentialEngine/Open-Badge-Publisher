<script lang="ts">
    ...
    import { canvasSelectedIssuerBadges, canvasSelectedRegion, canvasAccessToken } from "$lib/stores/badgeSourceStore.js";
    import {
        publisherUser,
    ...
    const updateCanvasCredential = async (c: CtdlApiCredential): Promise<boolean> => {
        const canvasBadge = $canvasSelectedIssuerBadges.find((cb) => cb.openBadgeId == c.Credential.CredentialId);
        const ctid = $ctdlPublicationResultStore[c.Credential.CredentialId]?.CTID;

        if (!canvasBadge)
            throw new Error('Could not identify source badge in Canvas.');

        const newAlignment: Alignment = {
            targetUrl: alignmentUrlForCredential(ctid),
            targetName: c.Credential.Name,
            targetDescription: `Additional information powered by the Credential Registry${PUBLIC_PUBLISHER_API_ENV_LABEL
        ...
        const requestData = {
            URL: `${canvasRegions.get($canvasSelectedRegion)?.apiDomain}/v2/badgeclasses/${canvasBadge.entityId}`,
            Method: 'PUT',
    ...
    $: {
        credentials = $credentialDrafts.filter((c) => {
            ...
            return [PubStatuses.SaveSuccess, PubStatuses.SourceUpdated].includes(status?.publicationStatus);
        });
        credentials.map(c => {
            updatePromises[c.Credential.CredentialId] = new Promise((resolve, reject) => {resolve(false);});
        })
			...
			{#each credentials as credential}
				<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="py-4 px-6">
                        ...
                        <div class="inline-block"><Check height="16" width="16" /></div> Saved
                        {:else}
                            ...
                            {:then value}
                                ...
                                <div class="inline-block"><Check height="16" width="16" /></div> Saved
                                {:else}
                                    <Button class="ughbutton" on:click={() => {updatePromises[credential.Credential.CredentialId] = updateCanvasCredential(credential)}}>Save</Button>
                                {/if}
                            {:catch error}
